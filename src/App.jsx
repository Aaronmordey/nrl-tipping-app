...
export default function App(){
  ...
  async function loadRegistrationSetting(){
    ...
    try{
      const {data,error}=await supabase.from("app_settings").select("value").eq("key","registration_open").single();
  ...
  async function setRegistrationSetting(open){
    ...
    if(error)setAuthError(error.message);
      ...
      setRegistrationOpen(open);
      setNotice(open?"Registration opened.":"Registration closed. Players can still join by admin invite.");
    }
    setSaving(false);
  }
...
function updateTip(gameId,update){
  ...
  return <div className="min-h-screen bg-slate-950 text-white"><div className="absolute inset-0 overflow-hidden pointer-events-none"><div className="absolute -top-24 -right-24 h-80 w-80 rounded-full bg-emerald-500/20 blur-3xl"/><div className="absolute top-96 -left-24 h-72 w-72 rounded-full bg-sky-500/20 blur-3xl"/></div><main className="relative mx-auto max-w-7xl px-3 py-4 sm:px-6 sm:py-6 lg:px-8">
    ...
    <RoundSelector rounds={rounds} selectedRound={selectedRound} setSelectedRound={setSelectedRound} roundLocked={roundLocked} autoCurrentRound={autoCurrentRound}/>
...
function RoundSelector({rounds,selectedRound,setSelectedRound,roundLocked,autoCurrentRound}){return <div className="mb-4 flex flex-col gap-3 rounded-3xl border border-white/10 bg-white/5 p-3 backdrop-blur sm:flex-row sm:items-center sm:justify-between"><div className="flex flex-wrap gap-2"><button onClick={()=>setSelectedRound(autoCurrentRound)} className="rounded-2xl bg-sky-400 px-4 py-2 font-bold text-slate-950 hover:bg-sky-300">Current Round</button>{rounds.map(r=><button key={r} onClick={()=>setSelectedRound(r)} className={`rounded-2xl px-4 py-2 font-bold ${Number(selectedRound)===Number(r)?"bg-emerald-400 text-slate-950":"bg-white/10 text-slate-200 hover:bg-white/20"}`}>Round {r}</button>)}</div><div className={`inline-flex items-center gap-2 rounded-2xl px-4 py-2 text-sm font-bold ${roundLocked?"bg-amber-400/20 text-amber-100":"bg-emerald-400/20 text-emerald-100"}`}><Clock className="h-4 w-4"/> {roundLocked?"Whole round locked":"Tips open until first game starts"}</div></div>}
...
function GameTip({game,database,currentUser,draftTips,updateTip,saving}){const tip=draftTips.find(t=>t.player_id===currentUser.id&&t.game_id===game.id); const result=getResult(game); const points=scoreTip(tip,result); const locked=isGameLocked(game,database.games); return <Card className="overflow-hidden rounded-3xl border border-white/10 bg-white/10 text-white"><CardContent className="p-0"><div className="grid gap-4 p-5 md:grid-cols-[1fr_auto] md:items-center"><div><Badges game={game} locked={locked} result={result}/><div className="flex items-center gap-5"><TeamBadge team={game.home} logo={game.home_logo}/><div className="rounded-full bg-white/10 px-4 py-2 text-lg font-black text-slate-300">v</div><TeamBadge team={game.away} logo={game.away_logo}/></div>{tip&&<p className="mt-2 text-sm text-slate-300">Selected tip: <strong className="text-white">{tip.winner}</strong> by <strong className="text-white">{tip.margin}</strong></p>}{result&&<p className="mt-1 text-sm text-slate-300">Your score for this game: <strong className="text-white">{points} points</strong></p>}</div><div className="grid gap-3 sm:grid-cols-2 lg:min-w-[420px]"><PickButtons title="Winner" options={[game.home,game.away]} value={tip?.winner} disabled={locked||saving} onPick={winner=>updateTip(game.id,{winner})}/><PickButtons title="Margin" options={["1-12","13+"]} value={tip?.margin} disabled={locked||saving} onPick={margin=>updateTip(game.id,{margin})}/></div></div></CardContent></Card>}
